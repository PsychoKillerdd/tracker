...
export default function ActivityList({ activities, dispatch }: ActivityListProps) {
  ...
  const isEmptyac = useMemo(() => activities.length === 0, [activities])
        ...
        activities.map(activity => (
          <div key={activity.id} className='px-5 py-10 bg-white mt-5 flex justify-between shadow'>
            <div className='space-y-2'>
              <p className={`px-10 py-2 text-white uppercase font-bold ${activity.category === 1 ? 'bg-lime-500' : 'bg-orange-500'}`}>              
